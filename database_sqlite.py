...
class DatabaseManager:
    ...
    def _init_database(self):
        with self.lock:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_chat_messages_users ON chat_messages(from_user_id, to_user_id, sent_date)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_chat_messages_unread ON chat_messages(to_user_id, is_read)')
    ...
    def save_chat_message(self, from_user_id: int, to_user_id: int, message: str, message_type: str = 'text') -> bool:
        ...
        try:
            with self.lock:
                ...
                cursor.execute('''
                    INSERT INTO chat_messages (from_user_id, to_user_id, message, sent_date, message_type)
                    VALUES (?, ?, ?, ?, ?)
                ''', (from_user_id, to_user_id, message, now, message_type))
                
                conn.commit()
                conn.close()
            
            LOGGER(__name__).info(f"Chat message saved from {from_user_id} to {to_user_id}")
            return True
        except Exception as e:
    ...
    def get_unread_messages(self, user_id: int) -> int:
        ...
        try:
            ...
            return result['unread'] if result else 0
        except Exception as e:
    ...
    def get_user_conversations(self, user_id: int, limit: int = 50) -> List[Dict]:
        ...
        try:
            ...
            LOGGER(__name__).error(f"Error getting conversations: {e}")
            return []
    
    def mark_messages_as_read(self, user_id: int, from_user_id: int) -> bool:
        ...
        try:
            ...
            LOGGER(__name__).error(f"Error marking messages as read: {e}")
            return False
